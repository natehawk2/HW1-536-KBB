...
When using this model we make some assumptions being linearity, independence of observations, normal distributed residuals, and equal variance. These assumptions will be justified in the next section.
...
In order to use our model, we must check the linear assumptions that we made the about data. We first check linearity. This is shown by added-variable plots. Due to the high number of covariates in our model we show only a few variables below, while the rest can be found in the appendix. Note that these added-variable plots are linear so we assume linearity.


```{r echo = FALSE}
library(patchwork)

avPlot(model, variable = "Mileage")
avPlot(model, variable = "Sound")
avPlot(model, variable = "Leather")


```


To use a linear model we need the data to be independent. We can assume independence because the data were collected indpendently and they have no relationships to each other that would cause them to be dependent (ie. owned by the same person). Next we check for normality by looking at the standardized residuals. Below is shown a histogram of the standardized residuals. They look very bell-shaped. We also show a QQ plot that shows the residuals are normal. We assume normality.

```{r echo = FALSE}
library(ggplot2)

ggplot(mapping = aes(x = rstandard(model)))+ 
...
We now check for equal variance by looking at a scatterplot of the fitted values and standardized residuals. Because there are no trends or patterns to this data, we assume equal variance. With the assumptions for a linear model met, we will move on to evaluating model performance.

```{r echo = FALSE}
ggplot(mapping = aes(x = model$fitted.values, y = rstandard(model))) + 
  geom_point() + 
